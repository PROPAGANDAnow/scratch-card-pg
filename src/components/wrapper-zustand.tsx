...
import { useAppStore, useUserStore, useCardStore, useUIStore } from "~/stores";
...
const WrapperZustand: FC<{ children: React.ReactNode }> = ({ children }) => {
  // Zustand stores
  const {
    appBackground,
    appStats,
  ...
  const getWinnerGifFn = () => winnerGifRef.current;



  // Refetch function for user cards
  const refetchUserCardsFn = async () => {
    if (!publicKey) return;
      ...
      if (userCards) {
        ...
        if (selectedCard) {
          ...
          if (updatedSelectedCard) {
            setSelectedCard(updatedSelectedCard as unknown as Card);
          }
        }
      }
    } catch (error) {
  ...
  // Set functions in UI store
  useEffect(() => {
    setPlayWinSound(playWinSoundFn);
    setGetWinnerGif(getWinnerGifFn);
    setRefetchUserCards(refetchUserCardsFn);
  }, []);
  ...
  return (
    ...
    >
      <div className="h-full max-w-[400px] flex flex-col mx-auto items-center justify-between">
        ...
        <Bottom
          mode="normal" // TODO: Get swipableMode from app store
          loading={loading}
        />
      </div>
    </div>
